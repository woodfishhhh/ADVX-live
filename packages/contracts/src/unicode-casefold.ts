...
export function pythonUnicodeCaseFold(value: string): string {
  let folded = ''
  for (const character of value) {
    const codePoint = character.codePointAt(0)
    folded += codePoint === undefined
      ? character
      : CASE_FOLD_MAP.get(codePoint) ?? character
  }
  return folded
}
...
export function truncateUnicodeCodePoints(value: string, limit: number): string {
